...
*`r renderText({ paste("EU trade:", money(eu_noneu_data()[1,2])) })`*
`r renderText({ paste("Non EU trade:", money(eu_noneu_data()[2,2])) })`
...
renderTable({
  ...
  t <- eu_noneu_data() %>% 
        mutate(value = money(tradevalue)) %>% 
        select(eunoneu, value)
